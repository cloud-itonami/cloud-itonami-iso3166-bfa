  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Ouaga Fabrication SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Ouaga Fabrication SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest pme-preference-margin-mismatch-is-held-and-unoverridable
  (testing "claimed evaluated price doesn't match the independently recomputed Art. 122 margin -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:pme-preference-margin-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest ifu-unverified-is-held-and-unoverridable
  (testing "unverified IFU (Identifiant Financier Unique) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:ifu-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
