(ns marketentry.store
  "SSoT for the Burkina Faso (BFA) market-entry compliance actor, behind
...
(defn demo-data
  ...
  `:pme-status?` / `:base-price` / `:claimed-evaluated-price` are
  ground truth for the `pme-preference-margin-mismatch` flagship check
  (Décret n°2024-1748/PRES/PM/MEF Art. 122 PME bid-evaluation
  preference); `:requires-ifu?` / `:ifu-verified?` are ground truth for
  the conditional IFU check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Ouaga Fabrication SARL" :portal "eBurkindi"
             ...
             :requires-ifu? true :ifu-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BFA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "eBurkindi"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :pme-status? true :base-price 1000000 :claimed-evaluated-price 950000.0
             :requires-ifu? true :ifu-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Bobo Systèmes SA" :portal "eBurkindi"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BFA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Sahel Négoce Export Ltd" :portal "eBurkindi"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "BFA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Koudougou Logistique SARL" :portal "eBurkindi"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :pme-status? true :base-price 1000000 :claimed-evaluated-price 950000.0
             :requires-ifu? true :ifu-verified? false
             :drafted? false :submitted? false
             :jurisdiction "BFA" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               pme-status? base-price claimed-evaluated-price
                               requires-ifu? ifu-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? pme-status?)                   (assoc :engagement/pme-status? pme-status?)
    base-price                            (assoc :engagement/base-price base-price)
    claimed-evaluated-price               (assoc :engagement/claimed-evaluated-price claimed-evaluated-price)
    (some? requires-ifu?)                 (assoc :engagement/requires-ifu? requires-ifu?)
    (some? ifu-verified?)                 (assoc :engagement/ifu-verified? ifu-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/pme-status? :engagement/base-price :engagement/claimed-evaluated-price
   :engagement/requires-ifu? :engagement/ifu-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :pme-status? (boolean (:engagement/pme-status? m))
     :base-price (:engagement/base-price m)
     :claimed-evaluated-price (:engagement/claimed-evaluated-price m)
     :requires-ifu? (boolean (:engagement/requires-ifu? m))
     :ifu-verified? (boolean (:engagement/ifu-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-bfa"})
