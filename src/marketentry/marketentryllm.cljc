(ns marketentry.marketentryllm
  ...
  the Burkina Faso (BFA) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "pme-status?=" (:pme-status? e)
                        " base-price=" (:base-price e)
                        " claimed-evaluated-price=" (:claimed-evaluated-price e)
                        " ifu-verified?=" (:ifu-verified? e)
