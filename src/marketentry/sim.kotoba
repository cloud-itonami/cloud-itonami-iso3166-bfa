...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (BFA, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up pme-preference-margin-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (pme-preference-margin-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up ifu-unverified) ==")
    ...
    (println "== filing/submit eng-5 (ifu-unverified -> HARD hold) ==")
